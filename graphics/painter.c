...
graphics_pos_t painter_rotate_x(graphics_pos_t x, painter_iq_t angle)
{
    painter_iq_t res = iq15_cos_pu(angle) * x;
    res = iq15_round(res);
    return iq15_int(res);
}
                      ...
                      graphics_pos_t radius, painter_iq_t from_angle, painter_iq_t to_angle)
{
    ...
    if(to_angle - from_angle >= IQ15_2PI_PU){
    ...
    from_angle = iq15_angle_norm_pu(from_angle);
    to_angle = iq15_angle_norm_pu(to_angle);
    ...
    if(from_angle < IQ15_PI2_PU){
        dx = -1;
        dy = 1;
    }else if(from_angle < IQ15_PI_PU){
        dx = -1;
        dy = -1;
    }else if(from_angle < (IQ15_PI_PU + IQ15_PI2_PU)){
        dx = 1;
        dy = -1;
    }else{
        dx = 1;
