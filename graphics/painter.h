...
/**
 * Тип чисел с фиксированной запятой.
 */
typedef iq15_t painter_iq_t;
 ...
 * result = x * cos(angle);
 * @param x Координата X.
 * @param angle Угол поворота, относительные единицы.
 * @return Повёрнутая координата.
 */
EXTERN graphics_pos_t painter_rotate_x(graphics_pos_t x, painter_iq_t angle);
 ...
 * @return Повёрнутая координата.
 */
EXTERN graphics_pos_t painter_rotate_y(graphics_pos_t y, painter_iq_t angle);
 ...
 * @param angle Угол поворота, относительные единицы.
 */
EXTERN void painter_rotate(graphics_pos_t* x, graphics_pos_t* y, painter_iq_t angle);
 ...
 * @param from_angle Начальный угол дуги, относительные единицы.
 * @param to_angle Конечный угол дуги, относительные единицы.
 */
