...
typedef enum _spi_error {
    ...
    SPI_ERROR_TIMEOUT           = 4,//!< Тайм-аут приёма.
...
typedef struct _SPI_Bus {
    // DMA.
    uint dma_rx_channel;//!< Номер канала DMA для приёма.
    dreq_num_t dma_rx_dreq; //!< Номер источника запроса DMA для приёма.
    uint dma_rx_irq_index; //!< Номер IRQ канала DMA для приёма.
    uint dma_tx_channel;//!< Номер канала DMA для передачи.
    dreq_num_t dma_tx_dreq; //!< Номер источника запроса DMA для передачи.
    uint dma_tx_irq_index; //!< Номер IRQ канала DMA для передачи.
    // SPI.
    spi_hw_t* spi_device;//!< Периферия.
